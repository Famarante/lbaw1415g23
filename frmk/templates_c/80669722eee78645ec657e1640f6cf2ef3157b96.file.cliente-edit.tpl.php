...
<script>
    $(function() {
        ...
        $( "#address-cancel" ).on('click', function() {
            $( "#address-form" ).attr('hidden', 'hidden');
            $( "#add-address" ).show();
        });   
        $( document ).ready(function(){
            $.ajax({
                type: "POST",
                url: "../../api/get-address.php",
                dataType: "json",
                success: function(response){

                    for ( var i = 0, l = response.length; i < l; i++ ) {
                        $('.address-list').append('<li class="list-group-item" data-zipcode="' + response[i].codigo + '"><strong>' + response[i].codigo + '</strong> - ' + response[i].rua + '<ul class="list-group-submenu" data-street="' + response[i].rua + '"><li class="list-group-submenu-item danger"><span class="glyphicon glyphicon-remove"></span></li></ul></li>');
                    }
                    bind_events();
                }
            });
        ...
        var bind_events = function(){
            $('li .list-group-submenu-item span').on('click', function(event) {
                event.preventDefault();
                var rua = $(this).parent().parent().attr('data-street');
                var codigo = $(this).parent().parent().parent().attr('data-zipcode');
                $(this).parent().parent().parent().remove();
                $.ajax({
                    type: "POST",
                    url: "../../api/remove-address.php",
                    data: { morada:{ rua:rua, codigo:codigo } },
                    dataType: "json",
                    success: function(response){
...
api/add-address.php" method="post" hidden>
                <div class="form-group">
                    ...
                    <div class="col-md-8">
                        <input class="form-control" type="text" name="address" required autofocus>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-8">
                        <select class="form-control" name="country" id="country-selector" autocorrect="off" autocomplete="off" required>
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-8">
                        ...
                        <span></span>
                        <input type="reset" class="btn btn-default" id="address-cancel" value="Cancelar">
                    </div>
                </div>
            </form>
        </div>
